<div class="d-flex flex-lg-row-fluid w-lg-50 bgi-size-cover bgi-position-center order-1 order-lg-2"
style="background-image: url({{ asset('admin_assets') }}/media/logos/newloginback.png)">
<!--begin::Content-->
<div class="d-flex flex-column flex-center py-7 py-lg-15 px-5 px-md-15 w-100">
    <!--begin::Logo-->
    {{-- <a href="index.html" class="mb-0 mb-lg-12">
        <img alt="Logo" src="{{ asset('admin_assets') }}/media/logos/Logoevix.png"
            class="h-60px h-lg-75px" />
    </a> --}}
    <!--end::Logo-->
    <!--begin::Image-->
    {{-- <img class="d-none d-lg-block mx-auto w-275px w-md-50 w-xl-600px mb-10 mb-lg-20"
        src="{{ asset('admin_assets') }}/media/logos/hrloginback.png" alt="" /> --}}
    <!--end::Image-->
    <!--begin::Title-->
    <h1 class="d-none d-lg-block text-white fs-2qx fw-bolder text-center mb-7"> </h1>
    <!--end::Title-->
    <!--begin::Text-->
    <div class="d-none d-lg-block text-white fs-base text-center">
        <a href="#" class="opacity-75-hover text-warning fw-bold me-1"></a>
        <br />
        <a href="#" class="opacity-75-hover text-warning fw-bold me-1">
        <br />
    </div>
    <!--end::Text-->
</div>
<!--end::Content-->
</div>
