<div class="card-body p-0">
    <div class="table-responsive">
        <table class="table table-striped gy-7 gs-7" id="companies-table">
            <thead>
                <tr class="fw-semibold fs-6 text-gray-800 border-bottom border-gray-200">
                    <th>@lang('models/Companies.fields.id')</th>
                    <th>@lang('models/Companies.fields.code')</th>
                    <th>@lang('models/Companies.fields.name')</th>
                    <th>@lang('models/Companies.fields.city')</th>
                    <th>@lang('models/Companies.fields.phone')</th>
                    <th>@lang('models/Companies.fields.email')</th>
                    <th>@lang('models/Companies.fields.status')</th>
                    <th colspan="3" class="text-center">@lang('crud.action')</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($Companies as $Company)
                    <tr>
                        <td>{{ $Company->id }}</td>
                        <td>{{ $Company->code }}</td>
                        <td>{{ $Company->name }}</td>
                        <td>{{ optional($Company->city)->name ?? '—' }}</td>
                        <td>{{ $Company->phone ?? '—' }}</td>
                        <td>{{ $Company->email ?? '—' }}</td>
                        <td><span class="{{ $Company->status_badge }}">{{ $Company->status_text }}</span></td>
                        <td style="width: 120px">
                            {!! Form::open(['route' => ['Companies.destroy', $Company->id], 'method' => 'delete']) !!}
                            <div class='btn-group'>
                                @can('Companies.show')
                                    <a href="{{ route('Companies.show', [$Company->id]) }}"
                                        class='btn btn-icon btn-sm btn-primary btn-xs'>
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                @endcan
                                @can('Companies.edit')
                                    <a href="{{ route('Companies.edit', [$Company->id]) }}"
                                        class='btn btn-icon btn-sm btn-primary mx-1  btn-xs'>
                                        <i class="fa-solid fa-edit"></i>
                                    </a>
                                @endcan
                                @can('Companies.destroy')
                                    {!! Form::button('<i class="fa-solid fa-trash"></i>', [
                                        'type' => 'submit',
                                        'class' => 'btn btn-icon btn-sm btn-primary   btn-xs',
                                        'onclick' => "return confirm('Are you sure?')",
                                    ]) !!}
                                @endcan
                            </div>
                            {!! Form::close() !!}
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    </div>
    <div class="card-footer clearfix py-4 {{ $Companies->hasPages() ? '' : 'd-none' }}">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $Companies])
        </div>
    </div>
</div>
