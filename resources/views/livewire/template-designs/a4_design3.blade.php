                            <!-- ================== A4 DESIGN 3: ELEGANT STRIPE PREVIEW ================== -->
                            <div class="invoice-border shadow-sm" style="padding-bottom: 30px;">
                                <!-- Top banner with dark background -->
                                <div class="row align-items-center mb-0 p-4"
                                    style="background: #1e293b; color: #ffffff; border-radius: 4px 4px 0 0; margin-left: 0; margin-right: 0;">
                                    <div class="col-6 text-start">
                                        <h2 class="text-white fw-bolder mb-1">
                                            {{ $previewData['invoice_title_ar'] ?? 'فاتورة ضريبية' }}
                                        </h2>
                                        @if($templateConfig['enable_english'])
                                            <div class="text-light small">{{ $previewData['invoice_title_en'] ?? 'Tax Invoice' }}</div>
                                        @endif
                                    </div>
                                    <div class="col-6 text-end">
                                        <h3 class="text-white mb-0 fw-bold">#{{ $previewData['invoice_number'] }}</h3>
                                        @if($templateConfig['show_small_barcode'] && !empty($previewData['invoice_number']))
                                            <div class="d-inline-block mt-1">
                                                <img src="data:image/png;base64,{{ base64_encode((new \Picqer\Barcode\BarcodeGeneratorPNG())->getBarcode($previewData['invoice_number'], \Picqer\Barcode\BarcodeGeneratorPNG::TYPE_CODE_128)) }}" alt="Barcode" style="height: 25px; max-width: 100%; filter: invert(1);">
                                            </div><br>
                                        @endif
                                        <span class="text-light small">تاريخ الإصدار @if($templateConfig['enable_english']) (Issue Date) @endif: {{ $previewData['issue_date'] }}</span>
                                    </div>
                                </div>
                                ...
                                <div style="padding: 25px;">
                                    ...
                                    <div class="row align-items-center mb-5">
                                        <div class="col-4 text-start">
                                            ...
                                            @if($templateConfig['show_tax_number'])
                                                <div class="small fw-bold">الرقم الضريبي @if($templateConfig['enable_english']) (VAT ID) @endif:</div>
                                                <div class="fw-bolder fs-5 border-bottom border-dark d-inline-block">
                                    ...
                                    <div class="row g-10 mb-5">
                                        <div class="col-6">
                                            <h6 class="fw-bolder border-bottom border-dark pb-2 mb-3">بيانات المشتري @if($templateConfig['enable_english']) / Buyer Info @endif</h6>
                                        ...
                                        <div class="col-6 text-end">
                                            <h6 class="fw-bolder border-bottom border-dark pb-2 mb-3">بيانات المستند @if($templateConfig['enable_english']) / Document Stats @endif</h6>
                                            <table class="w-100">
                                                <tr>
                                    ...
                                    <table class="table-zatca mb-0">
                                        ...
                                        <tr>
                                            ...
                                            <th style="background: #1e293b !important; color: #ffffff !important; border: 1px solid #1e293b;"
                                                width="3%">#</th>
                                            @endif
                                            @if($templateConfig['show_item_image'])
                                            <th style="background: #1e293b !important; color: #ffffff !important; border: 1px solid #1e293b;"
                                                width="5%">صورة @if($templateConfig['enable_english']) <br><small>Image</small> @endif</th>
                                            @endif
                                            <th style="background: #1e293b !important; color: #ffffff !important; border: 1px solid #1e293b;"
                                                width="16%">اسم الصنف @if($templateConfig['enable_english']) <br><small>Item Name</small> @endif</th>
                                            <th style="background: #1e293b !important; color: #ffffff !important; border: 1px solid #1e293b;"
                                                width="12%">الوصف @if($templateConfig['enable_english']) <br><small>Description</small> @endif</th>
                                        ...
                                        <tbody>
                                            @foreach($previewData['items'] as $item)
                                                <tr>
                                                    @if($templateConfig['show_item_number'])
                                                    <td style="border: 1px solid #1e293b;">{{ $loop->iteration }}</td>
                                                    @endif
                                                    @if($templateConfig['show_item_image'])
                                                    <td style="border: 1px solid #1e293b;" class="text-center">
                                                        @if(!empty($item['image']))
                                                            <img src="{{ $item['image'] }}" style="max-height: 40px; max-width: 40px;">
                                                        @endif
                                                    </td>
                                                    @endif
                                                    <td style="border: 1px solid #1e293b;" class="text-start">
                                                    ...
                                                    <td style="border: 1px solid #1e293b;" class="text-start">
                                                        {{ $item['description'] }}</td>
                                                    @if($templateConfig['show_item_unit'])
                                                        <td style="border: 1px solid #1e293b;">{{ $item['unit_name'] }}</td>
                                                    @endif
                                                    <td style="border: 1px solid #1e293b;">{{ $item['unit_price'] }}</td>
                                                    <td style="border: 1px solid #1e293b;">{{ $item['quantity'] }}</td>
                                                    @if($templateConfig['show_item_discount'])
                                                    <td style="border: 1px solid #1e293b;" class="text-danger small">
                                                        {{ $item['discount'] }}</td>
                                                    @endif
                                                    @if($templateConfig['show_item_subtotal'])
                                                        <td style="border: 1px solid #1e293b;">{{ $item['unit_price'] }}</td>
                                                    @endif
                                                    @if($templateConfig['show_item_tax_percent'])
                                                    <td style="border: 1px solid #1e293b;">{{ $item['vat_rate'] }}%</td>
                                                    @endif
                                                    <td style="border: 1px solid #1e293b;">{{ $item['vat_amount'] }}</td>
                                                    @if($templateConfig['show_item_total_with_tax'])
                                                    <td style="border: 1px solid #1e293b;" class="fw-bolder">{{ $item['total'] }}
                                                    </td>
                                                    @endif
                                                </tr>
                                            @endforeach
