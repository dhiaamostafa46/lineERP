...
@section('title', __('store::models/st_settlements.singular'))
        ...
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">
                    @lang('store::models/st_settlements.singular') @lang('crud.detail')
                </h1>
                <!--end::Title-->
                <!--begin::Breadcrumb-->
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-muted">
                        <a href="{{ route('store.settlement.index') }}" class=" text-muted text-hover-primary">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                @if(isset($settlement))
                    @if($settlement->status == \Modules\Store\App\Models\StSettlement::STATUS_DRAFT)
                        ...
                        {!! Form::button('<i class="fa-solid fa-check-double fs-5 me-1"></i> تعميد', [
                            'type' => 'submit',
                    ...
                    @if($settlement->is_editable)
                        <a href="{{ route('store.settlement.edit', [$settlement->id]) }}"
                            class='btn btn-sm btn-primary' title="@lang('crud.edit')">
                            <i class="fa-solid fa-edit fs-5 me-1"></i> @lang('crud.edit')
                        </a>
                    @endif

                    @if($settlement->is_deletable)
                        {!! Form::open(['route' => ['store.settlement.destroy', $settlement->id], 'method' => 'delete', 'class' => 'd-inline']) !!}
                        {!! Form::button('<i class="fa-solid fa-trash fs-5 me-1"></i> ' . __('crud.delete'), [
                            'type' => 'submit',
                ...
                <a class="btn btn-sm btn-secondary float-right" href="{{ route('store.settlement.index') }}">
                    @lang('crud.back')
                </a>
            </div>
            <!--end::Actions-->
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row gap-1">
                        @include('store::settlements.show_fields')
                    </div>
                </div>
            </div>
        </div>
    ...
    window.addEventListener('DOMContentLoaded', function() {
        ...
        window.onafterprint = function() {
            setTimeout(function() {
                window.close();
                setTimeout(function() {
                    window.location.href = "{{ route('store.settlement.index') }}";
                }, 300);
            }, 100);
        };
