...
@section('title', __('store::models/st_stores.plural'))
            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    ...
                    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">
                        <h1>@lang('store::models/st_stores.plural')</h1>
                    </h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <!--begin::Item-->
                        <li class="breadcrumb-item text-muted">
                            @lang('store::models/st_stores.plural')
                        </li>
                        <!--end::Item-->
                    </ul>
                    <!--end::Breadcrumb-->
                ...
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    ...
                    @endcan
                    @can('store.stores.csv')
                        <a type="button" class="icon-btn" href="{{ route('store.stores.csv') }}"
                            style="background-color: transparent; border: 1px solid #cbd5e0; color: #4a5568; border-radius: 6px; padding: 8px 10px; cursor: pointer; transition: all 0.3s;">
                            <i class="fa-solid fa-file-csv" style="font-size: 14px;"></i>
                        </a>
                    @endcan
                    <!-- أيقونة Excel -->
                    @can('store.stores.excel')
                        <a type="button" class="icon-btn"   href="{{ route('store.stores.excel') }}"
                            style="background-color: transparent; border: 1px solid #cbd5e0; color: #4a5568; border-radius: 6px; padding: 8px 10px; cursor: pointer; transition: all 0.3s;">
                            <i class="fa-solid fa-file-excel" style="font-size: 14px;"></i>
                        </a>
                    @endcan
                    <!-- أيقونة النسخ -->
                    @can('store.stores.pdf')
                        <a type="button" class="icon-btn"  href="{{ route('store.stores.pdf') }}" target="_blank"
                            style="background-color: transparent; border: 1px solid #cbd5e0; color: #4a5568; border-radius: 6px; padding: 8px 10px; cursor: pointer; transition: all 0.3s;">
                            <i class="fa-solid fa-file-pdf" style="font-size: 14px;"></i>
                        </a>
                    @endcan
                    @can('store.stores.create')
                        <a class="btn btn-sm btn-primary float-right" href="{{ route('store.stores.create') }}">
                ...
                @if (true)
                    <div class="card shadow-sm my-3 " id="card-filter">
                        ...
                        <div id="kt_docs_card_collapsible" class="collapse">
                            {!! Form::open(['route' => 'store.stores.index', 'method' => 'GET']) !!}
                            <div class="card-body">
                                <div class="row">
                                    <!-- Name Field -->
                                    <div class="form-group col-sm-3">
                                        {!! Form::label('name', __('store::models/st_stores.fields.name') . ':') !!}
                                        {!! Form::text('name', request('name'), ['class' => 'form-control']) !!}
                                    </div>

                                    <div class="form-group col-md-3 mb-3">
                                        {!! Form::label('type', __('store::models/st_stores.fields.type') . ':') !!}
                                        <x-select2-input name="type" :placeholder="__('lang.select')" :list="$types"
                                            :selected_id="request('type')">
                                        </x-select2-input>
                                    </div>

                                    <div class="form-group col-md-3 mb-3">
                                        {!! Form::label('status', __('store::models/st_stores.fields.status') . ':') !!}
                                        <x-select2-input name="status" :placeholder="__('lang.select')" :list="$statuses"
                                ...
                                <div class="card-footer py-4 d-flex justify-content-end gap-2">
                                    ...
                                    <a class="btn btn-sm btn-secondary" href="{{ route('store.stores.index') }}">
