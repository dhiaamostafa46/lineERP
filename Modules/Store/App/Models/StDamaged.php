...
use Modules\Store\Database\Factories\StDamagedFactory;

class StDamaged extends Model
{
    use \App\Traits\BelongsToBranch;

      use SoftDeletes, ImageUploaderTrait;

    protected $table = 'st_damageds';

    protected $fillable = ['org_id', 'branch_id', 'user_id', 'document_number', 'document_date', 'store_id', 'status', 'attachment', 'type', 'total_items', 'total_quantity', 'total_value', 'approved_by', 'approved_at', 'notes', 'journal_entry_id'];
    ...
    public function setAttachmentAttribute($file)
    {
        try {
            if ($file) {
                if ($this->attachment) {
                    $this->deleteFile($this->attachment, 'store_damageds');
                }
                $fileName = $this->createFileName($file);
                $this->saveFileType($file, $fileName, 'store_damageds');
    ...
    {
        if ($this->attachment && File::exists(public_path('uploads/images/store_damageds/' . $this->attachment))) {
            return 'uploads/images/store_damageds/' . $this->attachment;
        }
        return null;
    }
    ...
    {
        $path = public_path('uploads/images/store_damageds/' . $this->attachment);
        ...
        return [
            ...
            'url' => asset('uploads/images/store_damageds/' . $this->attachment),
        ];
    }
    ...
    {
        return $this->hasMany(StDamagedItem::class, 'damaged_id' ,'id');
    }
