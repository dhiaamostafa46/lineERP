...
class StOpeningBalance extends Model
{
    ...
    public function setAttachmentAttribute($file)
    {
        try {
            if ($file) {
                if ($this->attachment) {
                    $this->deleteFile($this->attachment, 'store_opening_balances');
                }
                $fileName = $this->createFileName($file);
                $this->saveFileType($file, $fileName, 'store_opening_balances');
    ...
    {
        if ($this->attachment && File::exists(public_path('uploads/images/store_opening_balances/' . $this->attachment))) {
            return 'uploads/images/store_opening_balances/' . $this->attachment;
        }
        return null;
    }
    ...
    {
        $path = public_path('uploads/images/store_opening_balances/' . $this->attachment);
        ...
        return [
            ...
            'url' => asset('uploads/images/store_opening_balances/' . $this->attachment),
        ];
    }
    ...
    {
        return $this->hasMany(StOpeningBalanceItem::class, 'opening_balance_id');
    }
