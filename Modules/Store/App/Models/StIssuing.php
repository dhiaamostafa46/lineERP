...
class StIssuing extends Model
{
    use \App\Traits\BelongsToBranch;

    use SoftDeletes, ImageUploaderTrait;

    protected $table = 'st_issuings';
    ...
    public function setAttachmentAttribute($file)
    {
        try {
            if ($file) {
                if ($this->attachment) {
                    $this->deleteFile($this->attachment, 'store_issuings');
                }
                $fileName = $this->createFileName($file);
                $this->saveFileType($file, $fileName, 'store_issuings');
    ...
    {
        if ($this->attachment && File::exists(public_path('uploads/images/store_issuings/' . $this->attachment))) {
            return 'uploads/images/store_issuings/' . $this->attachment;
        }
        return null;
    }
    ...
    {
        $path = public_path('uploads/images/store_issuings/' . $this->attachment);
        ...
        return [
            ...
            'url' => asset('uploads/images/store_issuings/' . $this->attachment),
        ];
    }
    ...
    {
        return $this->hasMany(StIssuingItem::class, 'issuing_id');
    }
