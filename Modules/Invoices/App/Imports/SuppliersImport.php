...
class SuppliersImport implements ToModel, WithChunkReading, WithBatchInserts, WithValidation, SkipsOnError, SkipsEmptyRows, WithStartRow
{
    private $errors = [];
    ...
    {
        // 1. تحديد الحساب الأب للموردين من الربط المحاسبي
        $this->parentId = AccountMapping::getAccountId('supplier');
    }
    ...
    public function model(array $row)
    {
        try {
            ...
            if (empty($names['ar'])) {
                throw new \Exception('اسم المورد حقل إجباري.');
            }

            DB::beginTransaction();

            // البحث عن المورد بالاسم العربي
            $supplier = InvSupplier::whereHas('translations', function ($q) use ($names) {
            ...
            if ($supplier) {
                ...
                // إنشاء سجل المورد
                $supplier = InvSupplier::create(array_merge([
            ...
            Log::error('Supplier import error at row', [
                'data' => $row,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        Log::error('Supplier import process error: ' . $e->getMessage());
    }
