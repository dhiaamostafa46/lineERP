@php
    $totalBeforeDiscount = $purchaseOrder->items->sum(function($item) {
        return $item->quantity * $item->unit_price;
    });
    $org = $purchaseOrder->branch->organization ?? \App\Models\Organization::first();
    
    $invoiceTitleAr = __('invoices::models/purchase_orders.singular');
    $invoiceTitleEn = 'Purchase Order';

    // ZATCA Settings (for Branch Address)
    $zatcaSetting = \Modules\Invoices\App\Models\ZatcaSetting::resolveForBranch($purchaseOrder->branch_id);
@endphp
...
<div class="saudi-invoice w-100">
    <div class="invoice-border shadow-sm">
        ...
        <div class="row align-items-center mb-5">
            <div class="col-4 text-start">
                <div class="qr-container">
                    @if($purchaseOrder->qr_code)
                        <img src="{{ (new \chillerlan\QRCode\QRCode)->render($purchaseOrder->qr_code) }}" alt="QR Code" style="width: 100%; height: 100%;">
                    @else
            ...
            <div class="col-4 text-center">
                ...
                <div class="fw-bold fs-4">@lang('invoices::models/purchase_orders.fields.invoice_number'): #{{ $purchaseOrder->invoice_number }}</div>
                    @if($purchaseOrder->invoice_number)
                        <img src="data:image/png;base64,{{ base64_encode((new \Picqer\Barcode\BarcodeGeneratorPNG())->getBarcode($purchaseOrder->invoice_number, \Picqer\Barcode\BarcodeGeneratorPNG::TYPE_CODE_128)) }}" alt="Barcode" style="max-width: 250px; height: 40px;">
                    @endif
            </div>
        ...
        <div class="row g-10 mb-5">
            <div class="col-6">
                <h6 class="fw-bolder border-bottom border-dark pb-1 mb-3">@lang('invoices::models/purchase_orders.ui.supplier_info') / Supplier Info</h6>
                <table class="w-100">
                    <tr>
                        <td class="data-label" width="45%">الاسم / Name:</td>
                        <td class="data-value fw-bolder">{{ $purchaseOrder->supplier->name ?? '---' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label">الرقم الضريبي / VAT ID:</td>
                        <td class="data-value">{{ $purchaseOrder->supplier->vat_number ?? '---' }}</td>
                    </tr>
                    @php
                        $supplierAddress = '';
                        if($purchaseOrder->supplier) {
                            $supplierAddressParts = array_filter([
                                $parseVal($purchaseOrder->supplier->building_number),
                                $parseVal($purchaseOrder->supplier->street),
                                $parseVal($purchaseOrder->supplier->district),
                                $parseVal($purchaseOrder->supplier->city),
                                $parseVal($purchaseOrder->supplier->country),
                                $parseVal($purchaseOrder->supplier->postal_code),
                                $parseVal($purchaseOrder->supplier->address)
            ...
            <div class="col-6 text-end">
                <h6 class="fw-bolder border-bottom border-dark pb-1 mb-3">@lang('invoices::models/purchase_orders.ui.order_stats') / Order Stats</h6>
                <table class="w-100">
                    <tr>
                        <td class="data-label text-start" width="45%">@lang('invoices::models/purchase_orders.fields.issue_date') / Date:</td>
                        <td class="data-value text-start fw-bolder">{{ $purchaseOrder->issue_date ? $purchaseOrder->issue_date->format('Y-m-d') : '' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label text-start">الفرع أو المستودع / Branch:</td>
                        <td class="data-value text-start">{{ $purchaseOrder->branch->name ?? '---' }}</td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="row mt-8 g-10 justify-content-end">
            ...
            <div class="col-7 no-print">
                <div class="p-4 border border-dark h-100">
                    <h6 class="fw-bolder border-bottom border-dark pb-2 mb-3">@lang('invoices::models/purchase_orders.ui.terms_and_conditions')</h6>
                    <div class="text-muted small italic">
                        {{ $purchaseOrder->notes ?: __('invoices::models/purchase_orders.ui.no_notes') }}
                    </div>
                </div>
            </div>
            ...
            <div class="col-5">
                <table class="w-100 border border-dark border-2">
                    ...
                    <tr>
                        ...
                        <td class="p-2 border-bottom border-dark text-end text-danger fw-bold">- {{ number_format($purchaseOrder->total_discount, 2) }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">الضريبة (VAT)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ $purchaseOrder->total_vat_formatted }}</td>
                    </tr>
                    <tr style="background: #f0f0f0; color: #000; border-top: 2px solid #000;">
                        ...
                        <td class="p-3 text-end fw-bolder fs-3">{{ $purchaseOrder->total_inclusive_vat_formatted }} </td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="mt-8 text-center small text-muted no-print">
            ...
            <div class="d-flex justify-content-between px-3 fw-bold text-dark">
                <span>إنشاء: {{ $purchaseOrder->createdBy->name ?? '---' }}</span>
                <span>الحالة: {{ $purchaseOrder->status_text }}</span>
            </div>
            <div class="mt-2 text-center" style="font-size: 0.7rem;">
                تم استخراج هذا الطلب من نظام EVIX ERP
            </div>
        </div>
    </div>
</div>
