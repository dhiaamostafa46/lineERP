<tr class="item-row text-center">
    ...
    <td>
        <select name="items[{{ $index }}][unit]" class="form-select" onchange="updateRowPrice(this)">
            @if($item->product && $item->product->units && count($item->product->units) > 0)
                ...
                @endforeach
            @else
                <option value="{{ $item->unit ?? __('invoices::models/purchase_orders.ui.piece') }}" selected>
                    {{ $item->unit ?? __('invoices::models/purchase_orders.ui.piece') }}
                </option>
            @endif
        </select>
    </td>
    ...
    <td>
        <div class="input-group">
            ...
            <select name="items[{{ $index }}][discount_type]" class="form-select item-discount-type" style="max-width: 70px" onchange="calcTotals()">
                <option value="fixed" {{ ($item->discount_type ?? 'fixed') == 'fixed' ? 'selected' : '' }}>{{ __('invoices::models/purchase_orders.fields.currency_symbol') }}</option>
