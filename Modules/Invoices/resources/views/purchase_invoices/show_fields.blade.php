@php
    ...
    $totalBeforeDiscount = $purchaseInvoice->items->sum(function($item) {
        return $item->quantity * $item->unit_price;
    });
    $org = $purchaseInvoice->branch->organization ?? \App\Models\Organization::first();
    
    // Determine Invoice Category
    $invoiceTitleAr = __('invoices::models/purchase_invoices.fields.type_inv_' . $purchaseInvoice->type_inv);
    $invoiceTitleEn = $purchaseInvoice->type_inv == 1 ? 'Purchase Invoice' : 'Purchase Tax Invoice';

    // ZATCA Settings (for Branch Address)
    $zatcaSetting = \Modules\Invoices\App\Models\ZatcaSetting::resolveForBranch($purchaseInvoice->branch_id);
@endphp
...
<div class="saudi-invoice w-100">
    <div class="invoice-border shadow-sm">
        ...
        <div class="row align-items-center mb-5">
            <div class="col-4 text-start">
                <div class="qr-container">
                    @if($purchaseInvoice->qr_code)
                        <img src="{{ (new \chillerlan\QRCode\QRCode)->render($purchaseInvoice->qr_code) }}" alt="QR Code" style="width: 100%; height: 100%;">
                    @else
            ...
            <div class="col-4 text-center">
                ...
                <div class="fw-bold fs-4">@lang('invoices::models/purchase_invoices.fields.invoice_number'): #{{ $purchaseInvoice->invoice_number }}</div>
                    @if($purchaseInvoice->invoice_number)
                        <img src="data:image/png;base64,{{ base64_encode((new \Picqer\Barcode\BarcodeGeneratorPNG())->getBarcode($purchaseInvoice->invoice_number, \Picqer\Barcode\BarcodeGeneratorPNG::TYPE_CODE_128)) }}" alt="Barcode" style="max-width: 250px; height: 40px;">
                    @endif
            </div>
            <div class="col-4 text-end">
                ...
                @if(isset($org->logo) && $org->logo)
                    <div class="mb-2">
                        <img src="{{ asset($org->logo) }}" alt="Logo" style="max-height: 60px;">
                    </div>
                @endif
                ...
                @elseif(isset($org->national_address) && $org->national_address)
                    <div class="small mt-1">{{ $parseVal($org->national_address) }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="row g-10 mb-5">
            <div class="col-6">
                <h6 class="fw-bolder border-bottom border-dark pb-1 mb-3">@lang('invoices::models/purchase_invoices.ui.supplier_info') / Supplier Info</h6>
                <table class="w-100">
                    <tr>
                        <td class="data-label" width="45%">الاسم / Name:</td>
                        <td class="data-value fw-bolder">{{ $purchaseInvoice->supplier->name ?? '---' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label">الرقم الضريبي / VAT ID:</td>
                        <td class="data-value">{{ $purchaseInvoice->supplier->vat_number ?? '---' }}</td>
                    </tr>
                    @php
                        $supplierAddress = '';
                        if($purchaseInvoice->supplier) {
                            $supplierAddressParts = array_filter([
                                $parseVal($purchaseInvoice->supplier->building_number),
                                $parseVal($purchaseInvoice->supplier->street),
                                $parseVal($purchaseInvoice->supplier->district),
                                $parseVal($purchaseInvoice->supplier->city),
                                $parseVal($purchaseInvoice->supplier->country),
                                $parseVal($purchaseInvoice->supplier->postal_code),
                                $parseVal($purchaseInvoice->supplier->address)
                    ...
                    <tr>
                        ...
                        <td class="data-value text-muted small">{{ $supplierAddress ?: '---' }}</td>
                    </tr>
                </table>
            </div>
            <div class="col-6 text-end">
                <h6 class="fw-bolder border-bottom border-dark pb-1 mb-3">@lang('invoices::models/purchase_invoices.ui.invoice_stats') / Invoice Stats</h6>
                <table class="w-100">
                    <tr>
                        <td class="data-label text-start" width="45%">@lang('invoices::models/purchase_invoices.fields.issue_date') / Date:</td>
                        <td class="data-value text-start fw-bolder">{{ $purchaseInvoice->issue_date ? $purchaseInvoice->issue_date->format('Y-m-d') : '' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label text-start">الفرع أو المستودع / Branch:</td>
                        <td class="data-value text-start">{{ $purchaseInvoice->branch->name ?? '---' }}</td>
                    </tr>
                    @if($purchaseInvoice->supplier_invoice_number)
                    <tr>
                        <td class="data-label text-start">@lang('invoices::models/purchase_invoices.fields.supplier_invoice_number'):</td>
                        <td class="data-value text-start fw-bold">#{{ $purchaseInvoice->supplier_invoice_number }}</td>
                    </tr>
                    @endif
                </table>
            </div>
        ...
        <table class="table-zatca">
            ...
            <tbody>
                ...
                @php
                    ...
                    $taxableAmount = ($item->quantity * $item->unit_price) - $discountAmt;
                    $vatAmount = (isset($item->vat_amount) && $item->vat_amount > 0) ? $item->vat_amount : ($taxableAmount * ($item->vat_rate / 100));
                @endphp
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td class="text-start">
                    ...
                    <td class="text-danger small">{{ $discountAmt > 0 ? number_format($discountAmt, 2) : '-' }}</td>
            ...
            <div class="col-5">
                <table class="w-100 border border-dark border-2">
                    ...
                    <tr>
                        ...
                        <td class="p-2 border-bottom border-dark text-end text-danger fw-bold">- {{ number_format($purchaseInvoice->total_discount, 2) }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">الصافي الخاضع للضريبة (Taxable Net)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">{{ number_format($purchaseInvoice->total_exclusive_vat, 2) }}</td>
                    </tr>
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">الضريبة (VAT)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ $purchaseInvoice->total_vat_formatted }}</td>
                    </tr>
                    <tr style="background: #f0f0f0; color: #000; border-top: 2px solid #000;">
                        ...
                        <td class="p-3 text-end fw-bolder fs-3">{{ $purchaseInvoice->total_inclusive_vat_formatted }} </td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="mt-8 text-center small text-muted no-print">
            ...
            <div class="d-flex justify-content-between px-3 fw-bold text-dark">
                <span>إنشاء: {{ $purchaseInvoice->createdBy->name ?? '---' }}</span>
                <span>الحالة: {{ $purchaseInvoice->status_text }}</span>
            </div>
            <div class="mt-2 text-center" style="font-size: 0.7rem;">
                تم استخراج هذه الفاتورة من نظام EVIX ERP
            </div>
        </div>
    </div>
</div>
