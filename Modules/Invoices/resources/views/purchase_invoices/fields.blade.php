    ...
    <div class="card border-0 rounded-3 shadow-sm mb-4 bg-white">
        <div class="card-header py-3 px-4 bg-transparent border-bottom">
            <h5 class="fw-bold text-dark mb-0 d-flex align-items-center gap-2">
                ...
                {{ __('invoices::models/purchase_invoices.ui.basic_info') }}
            </h5>
        </div>
        <div class="card-body p-4">
            <div class="row g-4">
                <div class="col-md-3">
                    <label
                        class="form-label fw-bold small text-muted">{{ __('invoices::models/purchase_invoices.fields.branch_id') }}
                        <span class="text-danger">*</span></label>
                    <x-select2-input name="store_id" :placeholder="__('invoices::models/purchase_invoices.fields.branch_id_placeholder')" :list="$stores" :selected_id="old('store_id', $purchaseInvoice?->store_id ?? '')">
                    </x-select2-input>

                </div>
                <div class="col-md-3">
                    <label
                        class="form-label fw-bold small text-muted">{{ __('invoices::models/purchase_invoices.fields.supplier_id') }}
                        <span class="text-danger">*</span></label>
                    {!! Form::select('supplier_id', [], old('supplier_id', $purchaseInvoice?->supplier_id ?? ''), ['class' => 'form-select select2-ajax-suppliers', 'data-selected' => old('supplier_id', $purchaseInvoice?->supplier_id ?? ''), 'placeholder' => __('invoices::models/purchase_invoices.fields.supplier_id_placeholder')]) !!}
                </div>
                <div class="col-md-2">
                    <label
                        class="form-label fw-bold small text-muted">{{ __('invoices::models/purchase_invoices.fields.supplier_invoice_number') }}</label>
                    {!! Form::text(
                        'supplier_invoice_number',
                        old('supplier_invoice_number', $purchaseInvoice?->supplier_invoice_number ?? null),
                        ['class' => 'form-control', 'placeholder' => 'INV-2024'],
                    ) !!}
                </div>
                <div class="col-md-2">
                    <label
                        class="form-label fw-bold small text-muted">{{ __('invoices::models/purchase_invoices.fields.issue_date') }}
                        <span class="text-danger">*</span></label>
                    {!! Form::datetimeLocal('issue_date', old('issue_date', $purchaseInvoice?->issue_date ?? now()), [
                        'class' => 'form-control',
                        'required',
                    ]) !!}
                </div>
                <div class="col-md-2">
                    ...
                    <select name="status" class="form-control" required>
                        @foreach($statuses ?? [1 => 'مسودة', 2 => 'معتمدة'] as $value => $label)
                            @php
                                $canDraft = auth()->user()->can('invoices.purchase.draft');
                                $canApprove = auth()->user()->can('invoices.purchase.approve');
                                $hasNeither = !$canDraft && !$canApprove;
                                $showOption = ($value == 1 && ($canDraft || $hasNeither)) || ($value == 2 && ($canApprove || $hasNeither));
                            @endphp
                            @if($showOption)
                                <option value="{{ $value }}" {{ old('status', $purchaseInvoice->status ?? ($canDraft ? 1 : 2)) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endif
                        @endforeach
            ...
            <input type="hidden" name="from_po_id" value="{{ old('from_po_id', $purchaseInvoice?->from_po_id ?? '') }}">
            {{-- <input type="hidden" name="invoice_number" value="{{ old('invoice_number', $purchaseInvoice?->invoice_number ?? mt_rand(100000, 999999)) }}"> --}}
        </div>
    </div>
                    ...
                    <ul class="nav nav-pills nav-pills-custom gap-3 p-4 pb-0" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link active d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_payments">
                                ...
                                {{ __('invoices::models/purchase_invoices.ui.payment_details') }}
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_discount"> 
                                ...
                                {{ __('invoices::models/purchase_invoices.ui.invoice_discount') }}
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_cost_center">
                                ...
                                {{ __('invoices::models/purchase_invoices.ui.cost_center') }}
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_attachments">
                                ...
                                {{ __('invoices::models/purchase_invoices.ui.attachments') }}
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade" id="tab_discount" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label
                                        class="form-label fw-bold small">{{ __('invoices::models/purchase_invoices.fields.additional_discount_value') }}</label>
                                    <div class="input-group">
                                        {!! Form::number('number_discount', old('number_discount', $purchaseInvoice?->number_discount ?? 0), [
                                            'class' => 'form-control',
                                        ...
                                        <select name="type_discount" id="total_invoice_discount_type"
                                            ...
                                            <option value="2">
                                                {{ __('invoices::models/purchase_invoices.fields.discount_type_fixed') }}
                                            </option>
                        ...
                        <div class="tab-pane fade" id="tab_cost_center" role="tabpanel">
                            <div class="col-md-4">
                                <label
                                    class="form-label fw-bold small">{{ __('invoices::models/purchase_invoices.fields.cost_center_id') ?? '' }}</label>
                                <x-select2-input name="cost_center_id" :placeholder="__('invoices::models/purchase_invoices.fields.cost_center_id') ?? '---'" :list="$cost_centers ?? []"
                                    :selected_id="old('cost_center_id', $purchaseInvoice?->cost_center_id ?? '')">
                                </x-select2-input>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="tab_attachments" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    <label
                                        class="form-label fw-bold small text-muted">{{ __('invoices::models/purchase_invoices.fields.notes') ?? 'الملاحظات' }}</label>
                                    {!! Form::textarea('notes', old('notes', $purchaseInvoice?->notes ?? null), [
                                        'class' => 'form-control',
                                        'rows' => 2,
                                        'placeholder' => __('invoices::models/purchase_invoices.fields.notes_placeholder') ?? 'إضافة ملاحظات إضافية...',
                                    ]) !!}
                                </div>

                                <div class="col-md-12">
                                    <label
                                        class="form-label fw-bold small">{{ __('invoices::models/purchase_invoices.ui.attachments') }}</label>
                                    <div
                                        ...
                                        @if (isset($purchaseInvoice) && $purchaseInvoice->file_url)
                                            <div class="mt-3">
                                                <a href="{{ $purchaseInvoice->file_url }}" target="_blank"
                        ...
                        <div class="tab-pane fade" id="tab_shipping" role="tabpanel">
                            <div class="row g-4">
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        {!! Form::number('shipping_cost', old('shipping_cost', $purchaseInvoice?->shipping_cost ?? 0), [
                                            'class' => 'form-control',
                                            'id' => 'shipping_cost',
                                            'step' => '0.01',
                                            'oninput' => 'calcTotals()',
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="shipping_tax_id" id="shipping_tax_id" class="form-select" onchange="calcTotals()">
                                        ...
                                        @foreach($taxes ?? [] as $tax_id => $name)
                                            ...
                                            <option value="{{ $tax_id }}" data-rate="{{ $taxRate }}" {{ old('shipping_tax_id', $purchaseInvoice?->shipping_tax_id ?? null) == $tax_id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="shipping_vat_rate" id="shipping_vat_rate_hidden" value="{{ old('shipping_vat_rate', $purchaseInvoice?->shipping_vat_rate ?? 0) }}">
                                </div>
            ...
            <script>
                $(document).ready(function() {
                    @if(session()->has('smart_import_data'))
                        @php $smartData = session('smart_import_data'); @endphp
                        @foreach($smartData['items'] as $item)
                            addItemBlock(
                                "{{ $item['id'] }}", 
                                "{{ $item['name'] }}", 
                                [], 
                        ...
                        @endforeach
                        @php session()->forget('smart_import_data'); @endphp
                    @endif
                });
            </script>
        @endpush
