@php
    $totalBeforeDiscount = $purchaseReturn->items->sum(function($item) {
        return $item->quantity * $item->unit_price;
    });
    $org = $purchaseReturn->branch->organization ?? \App\Models\Organization::first();
    
    $invoiceTitleAr = __('invoices::models/purchase_return_invoices.ui.return_note');
    $invoiceTitleEn = 'Purchase Return Invoice';

    // ZATCA Settings (for Branch Address)
    $zatcaSetting = \Modules\Invoices\App\Models\ZatcaSetting::resolveForBranch($purchaseReturn->branch_id ?? $purchaseReturn->store_id);
@endphp
...
<div class="saudi-invoice w-100">
    <div class="invoice-border shadow-sm">
        ...
        <div class="row align-items-center mb-5">
            <div class="col-4 text-start">
                <div class="qr-container">
                    @if($purchaseReturn->qr_code)
                        <img src="{{ (new \chillerlan\QRCode\QRCode)->render($purchaseReturn->qr_code) }}" alt="QR Code" style="width: 100%; height: 100%;">
                    @else
            ...
            <div class="col-4 text-center">
                ...
                <div class="fw-bold fs-4">@lang('invoices::models/purchase_return_invoices.fields.invoice_number'): #{{ $purchaseReturn->invoice_number }}</div>
                    @if($purchaseReturn->invoice_number)
                        <img src="data:image/png;base64,{{ base64_encode((new \Picqer\Barcode\BarcodeGeneratorPNG())->getBarcode($purchaseReturn->invoice_number, \Picqer\Barcode\BarcodeGeneratorPNG::TYPE_CODE_128)) }}" alt="Barcode" style="max-width: 250px; height: 40px;">
                    @endif
                @if($purchaseReturn->parent)
                    <div class="small text-muted mt-1">
                        @lang('invoices::models/purchase_return_invoices.fields.parent_id'):
                        <a href="{{ route('invoices.purchase.show', $purchaseReturn->parent_id) }}" class="text-primary fw-bold">
        ...
        <div class="row g-10 mb-5">
            <div class="col-6">
                <h6 class="fw-bolder border-bottom border-dark pb-1 mb-3">@lang('invoices::models/purchase_return_invoices.ui.supplier_info') / Supplier Info</h6>
                <table class="w-100">
                    <tr>
                        <td class="data-label" width="45%">الاسم / Name:</td>
                        <td class="data-value fw-bolder">{{ $purchaseReturn->supplier->name ?? '---' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label">الرقم الضريبي / VAT ID:</td>
                        <td class="data-value">{{ $purchaseReturn->supplier->vat_number ?? '---' }}</td>
                    </tr>
                    @php
                        $supplierAddress = '';
                        if($purchaseReturn->supplier) {
                            $supplierAddressParts = array_filter([
                                $purchaseReturn->supplier->building_number,
                                $purchaseReturn->supplier->street,
                                $purchaseReturn->supplier->district,
                                $purchaseReturn->supplier->city,
                                $purchaseReturn->supplier->country,
                                $purchaseReturn->supplier->postal_code,
            ...
            <div class="col-6 text-end">
                <h6 class="fw-bolder border-bottom border-dark pb-1 mb-3">@lang('invoices::models/purchase_return_invoices.ui.return_info') / Return Stats</h6>
                <table class="w-100">
                    <tr>
                        <td class="data-label text-start" width="45%">@lang('invoices::models/purchase_return_invoices.fields.issue_date') / Date:</td>
                        <td class="data-value text-start fw-bolder">{{ $purchaseReturn->issue_date ? $purchaseReturn->issue_date->format('Y-m-d') : '' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label text-start">الفرع أو المستودع / Branch:</td>
                        <td class="data-value text-start">{{ $purchaseReturn->store->name ?? '---' }}</td>
                    </tr>
                    @if($purchaseReturn->return_reason)
                    <tr>
                        <td class="data-label text-start">@lang('invoices::models/purchase_return_invoices.fields.return_reason'):</td>
                        <td class="data-value text-start text-muted small">{{ $purchaseReturn->return_reason }}</td>
                    </tr>
                    @endif
                </table>
            </div>
            ...
            <div class="col-5">
                <table class="w-100 border border-dark border-2">
                    ...
                    <tr>
                        ...
                        <td class="p-2 border-bottom border-dark text-end text-danger fw-bold">- {{ number_format($purchaseReturn->total_discount, 2) }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">الضريبة (VAT)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ $purchaseReturn->total_vat_formatted }}</td>
                    </tr>
                    <tr style="background: #f0f0f0; color: #000; border-top: 2px solid #000;">
                        ...
                        <td class="p-3 text-end fw-bolder fs-3">{{ $purchaseReturn->total_inclusive_vat_formatted }} </td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="mt-8 text-center small text-muted no-print">
            ...
            <div class="d-flex justify-content-between px-3 fw-bold text-dark">
                <span>إنشاء: {{ $purchaseReturn->createdBy->name ?? '---' }}</span>
                <span>الحالة: {{ $purchaseReturn->status_text }}</span>
            </div>
