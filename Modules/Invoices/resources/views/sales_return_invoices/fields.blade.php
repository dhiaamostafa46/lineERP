<div class="container-fluid py-4" id="sales-invoice-app">
    ...
    <div class="card border-0 rounded-3 shadow-sm mb-4 bg-white">
        <div class="card-header py-3 px-4 bg-transparent border-bottom">
            <h5 class="fw-bold text-dark mb-0 d-flex align-items-center gap-2">
                ...
                @lang('invoices::models/sales_return_invoices.ui.basic_info')
            </h5>
        </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold small text-muted">
                        @lang('invoices::models/sales_return_invoices.fields.branch_id')
                        <span class="text-danger">*</span>
                    </label>
                    <x-select2-input name="store_id"
                        :placeholder="__('invoices::models/sales_return_invoices.fields.branch_id_placeholder')"
                        :list="$stores"
                        :selected_id="old('store_id', $salesReturn?->store_id ?? '')">
                    </x-select2-input>
                </div>
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold small text-muted">
                        @lang('invoices::models/sales_return_invoices.fields.customer_id')
                        <span class="text-danger">*</span>
                    </label>
                    {!! Form::select('customer_id', [], old('customer_id', $salesReturn?->customer_id ?? $parentInvoice?->customer_id ?? ''), ['class' => 'form-select select2-ajax-customers', 'data-selected' => old('customer_id', $salesReturn?->customer_id ?? $parentInvoice?->customer_id ?? ''), 'placeholder' => __('invoices::models/sales_return_invoices.fields.customer_id_placeholder')]) !!}
                </div>

                @if(isset($selectedParentId) || isset($salesReturn->parent_id))
                {{-- حقل الربط المخفي في حال كان المرتجع من فاتورة محددة --}}
                <div class="col-md-3">
                    <label class="form-label fw-bold small text-muted">@lang('invoices::models/sales_return_invoices.fields.original_invoice_number')</label>
                    <input type="text" class="form-control form-control-solid bg-light" readonly 
                           value="{{ $parentInvoice?->invoice_number ?? $salesReturn?->parent?->invoice_number ?? old('parent_id', $selectedParentId ?? $salesReturn->parent_id) }}">
                </div>
                <input type="hidden" name="parent_id" value="{{ old('parent_id', $selectedParentId ?? $salesReturn->parent_id) }}">
                @endif
                ...
                <div class="col-md-3">
                    ...
                    <select name="status" class="form-control" required>
                        @foreach($statuses as $value => $label)
                            @php
                                $canDraft = auth()->user()->can('invoices.sales_return.draft');
                                $canApprove = auth()->user()->can('invoices.sales_return.approve');
                                $hasNeither = !$canDraft && !$canApprove;
                                $showOption = ($value == 1 && ($canDraft || $hasNeither)) || ($value == 2 && ($canApprove || $hasNeither));
                            @endphp
                            @if($showOption)
                                <option value="{{ $value }}" {{ old('status', $salesReturn->status ?? ($canDraft ? 1 : 2)) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endif
                        @endforeach
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold small text-muted">
                        @lang('invoices::models/sales_return_invoices.fields.issue_date')
                        <span class="text-danger">*</span>
                    </label>
                    {!! Form::datetimeLocal('issue_date', old('issue_date', $salesReturn?->issue_date ?? now()), [
                        'class' => 'form-control',
                        'required',
                    ]) !!}
                </div>
                ...
                {{-- سبب الإرجاع --}}
                <div class="col-md-6">
                    <label class="form-label fw-bold small text-muted">@lang('invoices::models/sales_return_invoices.fields.return_reason')</label>
                    {!! Form::textarea('return_reason',
                        old('return_reason', $salesReturn?->return_reason ?? ''),
                        ['class' => 'form-control', 'rows' => 2, 'placeholder' => __('invoices::models/sales_return_invoices.ui.return_reason_hint')]
                    ) !!}
                </div>
                ...
                {{-- <div class="col-md-3">
                    <label class="form-label fw-bold small text-muted">
                        @lang('invoices::models/sales_return_invoices.fields.customer_invoice_number')
                    </label>
                    {!! Form::text('customer_invoice_number',
                        old('customer_invoice_number', $salesReturn?->customer_invoice_number ?? null),
    ...
    @include('invoices::components.items_details', [
        'langPrefix' => 'invoices::models/sales_return_invoices',
        'invoice'    => $salesReturn ?? null,
    ])
                    ...
                    <ul class="nav nav-pills nav-pills-custom gap-3 p-4 pb-0" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link active d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_payments">
                                ...
                                @lang('invoices::models/sales_return_invoices.ui.payment_details')
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_discount">
                                ...
                                @lang('invoices::models/sales_return_invoices.ui.invoice_discount')
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_cost_center">
                                ...
                                {{ __('invoices::models/sales_return_invoices.ui.cost_center') }}
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_attachments">
                                ...
                                @lang('invoices::models/sales_return_invoices.ui.attachments')
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link d-flex align-items-center py-3 px-4 fs-7 fw-bold" data-bs-toggle="tab" href="#tab_shipping">
                                ...
                                {{ __('invoices::models/sales_return_invoices.fields.shipping_cost') }}
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="card-body p-4">
                    <div class="tab-content">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label class="form-label fw-bold small">
                                        @lang('invoices::models/sales_return_invoices.fields.additional_discount_value')
                                    </label>
                                    <div class="input-group">
                                        {!! Form::number('number_discount',
                                            old('number_discount', $salesReturn?->number_discount ?? 0),
                                        ...
                                        <select name="type_discount" id="total_invoice_discount_type"
                                            ...
                                            <option value="2">@lang('invoices::models/sales_return_invoices.fields.discount_type_fixed')</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="tab-pane fade" id="tab_cost_center" role="tabpanel">
                            <div class="col-md-4">
                                <label class="form-label fw-bold small">
                                    {{ __('invoices::models/sales_return_invoices.ui.cost_center') ?? 'مركز التكلفة' }}
                                </label>
                                <x-select2-input name="cost_center_id"
                                    :placeholder="__('invoices::models/sales_return_invoices.ui.cost_center') ?? '---'"
                                    :list="$cost_centers ?? []"
                                    :selected_id="old('cost_center_id', $salesReturn?->cost_center_id ?? '')">
                                </x-select2-input>
                            </div>
                        </div>
                        {{-- المرفقات --}}
                        <div class="tab-pane fade" id="tab_attachments" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    <label class="form-label fw-bold small">@lang('invoices::models/sales_return_invoices.ui.attachments')</label>
                                    <div class="upload-area p-4 border border-2 border-dashed rounded text-center bg-light position-relative">
                                        <label for="invoice_file" class="cursor-pointer d-block" style="cursor: pointer;">
                                            ...
                                            <p class="mb-1 fw-bold text-muted">@lang('invoices::models/sales_return_invoices.ui.upload_hint')</p>
                                        </label>
                                        ...
                                        @if(isset($salesReturn) && $salesReturn->file_url)
                                            <div class="mt-3">
                                                <a href="{{ $salesReturn->file_url }}" target="_blank" class="btn btn-sm btn-outline-info">
                                                    <i class="bi bi-eye me-1"></i> @lang('invoices::models/sales_return_invoices.ui.view_attachment')
                                                </a>
                                            </div>
                                        @endif
                                    </div>
                        ...
                        <div class="tab-pane fade" id="tab_shipping" role="tabpanel">
                            <div class="row g-4">
                                <div class="col-md-4">
                                    <label class="form-label fw-bold small text-muted">{{ __('invoices::models/sales_return_invoices.fields.shipping_cost') }}</label>
                                    <div class="input-group">
                                        {!! Form::number('shipping_cost', old('shipping_cost', $salesReturn?->shipping_cost ?? 0), [
                                            'class' => 'form-control',
                                            'id' => 'shipping_cost',
                                            'step' => '0.01',
                                            'oninput' => 'calcTotals()',
                                        ]) !!}
                                        <span class="input-group-text bg-light">{{ __('invoices::models/sales_return_invoices.fields.currency_symbol') }}</span>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-bold small text-muted">{{ __('invoices::models/sales_return_invoices.fields.shipping_vat_rate') }}</label>
                                    <select name="shipping_tax_id" id="shipping_tax_id" class="form-select" onchange="calcTotals()">
                                        ...
                                        @foreach($taxes ?? [] as $tax_id => $name)
                                            ...
                                            <option value="{{ $tax_id }}" data-rate="{{ $taxRate }}" {{ old('shipping_tax_id', $salesReturn?->shipping_tax_id ?? null) == $tax_id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="shipping_vat_rate" id="shipping_vat_rate_hidden" value="{{ old('shipping_vat_rate', $salesReturn?->shipping_vat_rate ?? 0) }}">
                                </div>
                            </div>
                        </div>
                    </div>
