@php
    ...
    $totalBeforeDiscount = $salesReturn->items->sum(function($item) {
        return $item->quantity * $item->unit_price;
    });
    $org = $salesReturn->branch->organization ?? \App\Models\Organization::first();
    
    // ZATCA Settings
    $zatcaSetting = \Modules\Invoices\App\Models\ZatcaSetting::resolveForBranch($salesReturn->branch_id);
    
    // Determine Invoice Category
    $isSimplified = empty($salesReturn->customer->vat_number);
    $invoiceTitleAr = $salesReturn->type_inv == 1 
        ? ($isSimplified ? 'مرتجع مبيعات مبسطة' : 'مرتجع مبيعات') 
        : ($isSimplified ? 'إشعار دائن ضريبي مبسط' : 'إشعار دائن ضريبي');
    $invoiceTitleEn = $salesReturn->type_inv == 1 
        ? ($isSimplified ? 'Simplified Sales Return Invoice' : 'Sales Return Invoice') 
    ...
    <div class="invoice-border shadow-sm">
        ...
        <div class="row align-items-center mb-5">
            <div class="col-4 text-start">
                <div class="qr-container">
                    @if($salesReturn->qr_code)
                        <img src="{{ (new \chillerlan\QRCode\QRCode)->render($salesReturn->qr_code) }}" alt="QR Code" style="width: 100%; height: 100%;">
                    @else
            ...
            <div class="col-4 text-center">
                ...
                <div class="fw-bold fs-4">رقم الفاتورة: #{{ $salesReturn->invoice_number }}</div>
                    @if($salesReturn->invoice_number)
                        <img src="data:image/png;base64,{{ base64_encode((new \Picqer\Barcode\BarcodeGeneratorPNG())->getBarcode($salesReturn->invoice_number, \Picqer\Barcode\BarcodeGeneratorPNG::TYPE_CODE_128)) }}" alt="Barcode" style="max-width: 250px; height: 40px;">
                    @endif
            </div>
            ...
            <div class="col-6">
                ...
                <table class="w-100">
                    <tr>
                        ...
                        <td class="data-value fw-bolder">{{ $salesReturn->customer->name ?? '---' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label">الرقم الضريبي / VAT ID:</td>
                        <td class="data-value">{{ $salesReturn->customer->vat_number ?? '---' }}</td>
                    </tr>
                    @php
                        $customerAddress = '';
                        if($salesReturn->customer) {
                            $customerAddressParts = array_filter([
                                $salesReturn->customer->building_number,
                                $salesReturn->customer->street,
                                $salesReturn->customer->district,
                                $salesReturn->customer->city,
                                $salesReturn->customer->country,
                                $salesReturn->customer->postal_code
            ...
            <div class="col-6 text-end">
                ...
                <table class="w-100">
                    <tr>
                        ...
                        <td class="data-value text-start fw-bolder">{{ $salesReturn->issue_date ? $salesReturn->issue_date->format('Y-m-d') : '' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label text-start">الفرع أو المستودع / Branch:</td>
                        <td class="data-value text-start">{{ $salesReturn->branch->name ?? '---' }}</td>
                    </tr>
                    @if($salesReturn->customer_invoice_number)
                    <tr>
                        <td class="data-label text-start">رقم فاتورة العميل:</td>
                        <td class="data-value text-start fw-bold">#{{ $salesReturn->customer_invoice_number }}</td>
                    </tr>
                    @endif
                </table>
            </div>
        ...
        <div class="row mt-8 g-10 justify-content-end">
            ...
            <div class="col-7 no-print">
                <div class="p-4 border border-dark h-100">
                    ...
                    @if($salesReturn->payments->count() > 0)
                    <table class="table table-sm table-borderless small mb-0">
                        ...
                        <tbody>
                            @foreach($salesReturn->payments as $payment)
                            <tr>
            ...
            <div class="col-5">
                <table class="w-100 border border-dark border-2">
                    ...
                    @if($salesReturn->total_discount > 0)
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">إجمالي الخصم (Discount)</td>
                        <td class="p-2 border-bottom border-dark text-end text-danger fw-bold">- {{ number_format($salesReturn->total_discount, 2) }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">الضريبة (VAT 15%)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ $salesReturn->total_vat_formatted }}</td>
                    </tr>
                    @if($salesReturn->shipping_cost > 0)
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">مصاريف الشحن (Shipping)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ number_format($salesReturn->shipping_cost + $salesReturn->shipping_vat_amount, 2) }}</td>
                    </tr>
                    @endif
                    <tr style="background: #f0f0f0; color: #000; border-top: 2px solid #000;">
                        ...
                        <td class="p-3 text-end fw-bolder fs-3">{{ $salesReturn->total_inclusive_vat_formatted }} </td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="mt-8 text-center small text-muted no-print">
            ...
            <div class="d-flex justify-content-between px-3 fw-bold text-dark">
                <span>إنشاء: {{ $salesReturn->createdBy->name ?? '---' }}</span>
                <span>الحالة: {{ $salesReturn->status_text }}</span>
            </div>
            ...
            {{-- ZATCA Status Section --}}
            @if($salesReturn->zatcaDetails)
                <div class="mt-4 p-3 border rounded text-start" style="background: #fcfcfc; border-style: dashed !important;">
                    ...
                    <div class="d-flex align-items-center">
                        <span class="{{ $salesReturn->status_badge }} me-3">{{ $salesReturn->status_text }}</span>
                        <span class="small text-muted">رقم العملية: {{ $salesReturn->zatcaDetails->request_id }}</span>
                    </div>
                    
                    @php $res = json_decode($salesReturn->zatcaDetails->response_payload, true); @endphp
