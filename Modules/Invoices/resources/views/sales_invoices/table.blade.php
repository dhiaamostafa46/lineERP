<div class="card-body p-0">
    <div class="table-responsive">
        ...
        @endphp
        <table class="table table-striped text-center gy-7 gs-7" id="db-sales-invoices-table">
            <thead>
                <tr class="fw-semibold fs-6 text-gray-800 border-bottom border-gray-200">
                    <th class="text-center">
                        <a href="{{ $buildSortUrl('invoice_number') }}" class="text-gray-800 text-hover-primary">
                            @lang('invoices::models/sales_invoices.fields.invoice_number') {!! $getSortIcon('invoice_number') !!}
                        </a>
                    </th>
                    <th class="text-center">
            ...
            <tbody>
                @foreach ($salesInvoices as $salesInvoice)
                    <tr>
                        ...
                        <td>{{ $salesInvoice->customer ? $salesInvoice->customer->name : '---' }}</td>
                        <td>{{ $salesInvoice->issue_date ? \Carbon\Carbon::parse($salesInvoice->issue_date)->format('Y-m-d') : '---' }}
                        </td>
                        <td>{{ number_format($salesInvoice->total_inclusive_vat, 2) }}</td>
                        <td>

                            <span class="{{ $salesInvoice->status_badge }}">{{ $salesInvoice->status_text }}</span>

                        </td>


                        <td style="width: 150px" class="table-action">
                            {!! Form::open(['route' => ['invoices.sales.destroy', $salesInvoice->id], 'method' => 'delete']) !!}
                            <div class='btn-group'>
                                <a href="{{ route('invoices.sales.show', [$salesInvoice->id]) }}"
                                ...
                                @if ($salesInvoice->status != \App\Models\invApp\SalesInvoice::STATUS_DRAFT && $salesInvoice->status != \App\Models\invApp\SalesInvoice::STATUS_RETURNED)
                                    <a href="{{ route('invoices.sales_debit.create', ['parent_id' => $salesInvoice->id]) }}"
                                        class='btn btn-sm btn-success float-right mx-1' title="إشعار مدين">
    ...
    <div class="card-footer clearfix py-4">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $salesInvoices])
        </div>
    </div>
</div>
