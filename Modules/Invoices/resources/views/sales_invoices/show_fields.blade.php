@php
    ...
    $totalBeforeDiscount = $salesInvoice->items->sum(function($item) {
        return $item->quantity * $item->unit_price;
    });
    $org = $salesInvoice->branch->organization ?? \App\Models\Organization::first();
    
    // ZATCA Settings
    $zatcaSetting = \Modules\Invoices\App\Models\ZatcaSetting::resolveForBranch($salesInvoice->branch_id);
    
    // Determine Invoice Category
    $isSimplified = empty($salesInvoice->customer->vat_number);
    $invoiceTitleAr = $salesInvoice->type_inv == 1 
        ? ($isSimplified ? 'فاتورة ضريبية مبسطة' : 'فاتورة ضريبية') 
        : ($isSimplified ? 'إشعار دائن ضريبي مبسط' : 'إشعار دائن ضريبي');
    $invoiceTitleEn = $salesInvoice->type_inv == 1 
    ...
    <div class="invoice-border shadow-sm">
        ...
        <div class="row align-items-center mb-5">
            <div class="col-4 text-start">
                <div class="qr-container">
                    @if($salesInvoice->qr_code)
                        <img src="{{ (new \chillerlan\QRCode\QRCode)->render($salesInvoice->qr_code) }}" alt="QR Code" style="width: 100%; height: 100%;">
                    @else
            ...
            <div class="col-4 text-center">
                ...
                <div class="fw-bold fs-4">رقم الفاتورة: #{{ $salesInvoice->invoice_number }}</div>
                    @if($salesInvoice->invoice_number)
                        <img src="data:image/png;base64,{{ base64_encode((new \Picqer\Barcode\BarcodeGeneratorPNG())->getBarcode($salesInvoice->invoice_number, \Picqer\Barcode\BarcodeGeneratorPNG::TYPE_CODE_128)) }}" alt="Barcode" style="max-width: 250px; height: 40px;">
                    @endif
            </div>
            ...
            <div class="col-6">
                ...
                <table class="w-100">
                    <tr>
                        ...
                        <td class="data-value fw-bolder">{{ $salesInvoice->customer->name ?? '---' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label">الرقم الضريبي / VAT ID:</td>
                        <td class="data-value">{{ $salesInvoice->customer->vat_number ?? '---' }}</td>
                    </tr>
                    @php
                        $customerAddress = '';
                        if($salesInvoice->customer) {
                            $customerAddressParts = array_filter([
                                $parseVal($salesInvoice->customer->building_number),
                                $parseVal($salesInvoice->customer->street),
                                $parseVal($salesInvoice->customer->district),
                                $parseVal($salesInvoice->customer->city),
                                $parseVal($salesInvoice->customer->country),
                                $parseVal($salesInvoice->customer->postal_code)
            ...
            <div class="col-6 text-end">
                ...
                <table class="w-100">
                    <tr>
                        ...
                        <td class="data-value text-start fw-bolder">{{ $salesInvoice->issue_date ? $salesInvoice->issue_date->format('Y-m-d') : '' }}</td>
                    </tr>
                    <tr>
                        <td class="data-label text-start">الفرع أو المستودع / Branch:</td>
                        <td class="data-value text-start">{{ $salesInvoice->branch->name ?? '---' }}</td>
                    </tr>
                    @if($salesInvoice->customer_invoice_number)
                    <tr>
                        <td class="data-label text-start">رقم فاتورة العميل:</td>
                        <td class="data-value text-start fw-bold">#{{ $salesInvoice->customer_invoice_number }}</td>
                    </tr>
                    @endif
                </table>
            </div>
        ...
        <div class="row mt-8 g-10 justify-content-end">
            ...
            <div class="col-7 no-print">
                <div class="p-4 border border-dark h-100">
                    ...
                    @if($salesInvoice->payments->count() > 0)
            ...
            <div class="col-5">
                <table class="w-100 border border-dark border-2">
                    ...
                    <tr>
                        ...
                        <td class="p-2 border-bottom border-dark text-end text-danger fw-bold">- {{ number_format($salesInvoice->total_discount, 2) }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td class="p-2 border-bottom border-dark data-label bg-row text-start text-dark">الضريبة (VAT 15%)</td>
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ $salesInvoice->total_vat_formatted }}</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="p-2 border-bottom border-dark text-end fw-bold">+ {{ number_format($salesInvoice->shipping_cost + $salesInvoice->shipping_vat_amount, 2) }}</td>
                    </tr>
                    @endif
                    <tr style="background: #f0f0f0; color: #000; border-top: 2px solid #000;">
                        ...
                        <td class="p-3 text-end fw-bolder fs-3">{{ $salesInvoice->total_inclusive_vat_formatted }} </td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="mt-8 text-center small text-muted no-print">
            ...
            <div class="d-flex justify-content-between px-3 fw-bold text-dark">
                <span>إنشاء: {{ $salesInvoice->createdBy->name ?? '---' }}</span>
                <span>الحالة: {{ $salesInvoice->status_text }}</span>
            </div>
            ...
            @if($salesInvoice->zatcaDetails)
                <div class="mt-4 p-3 border rounded text-start" style="background: #fcfcfc; border-style: dashed !important;">
                    ...
                    <div class="d-flex align-items-center">
                        <span class="{{ $salesInvoice->status_badge }} me-3">{{ $salesInvoice->status_text }}</span>
                        <span class="small text-muted">رقم العملية: {{ $salesInvoice->zatcaDetails->request_id }}</span>
                    </div>
                    
                    @php $res = json_decode($salesInvoice->zatcaDetails->response_payload, true); @endphp
