    ...
    <table class="table front-table text-start align-middle" id="db-kitchens-table" data-table="bulk">
        <thead>
            <tr>
                ...
                <th><x-table-sort column="barcode" title="BARCODE" /></th>
...
@if(method_exists($kitchens, 'hasPages') && $kitchens->hasPages())
    <div class="front-card-footer">
        <div class="fs-8 text-muted">
            Showing <span class="fw-bold text-gray-800">{{ $kitchens->firstItem() ?? 0 }}</span> to <span class="fw-bold text-gray-800">{{ $kitchens->lastItem() ?? 0 }}</span> of <span class="fw-bold text-gray-800">{{ $kitchens->total() }}</span> entries
        </div>
        <div>
