<div class="table-responsive">
    <table class="table front-table text-start" id="db-products-table">
        <thead>
            <tr>
                ...
                <th class="ps-2">@lang('basicdata::models/db_products.fields.name')</th>
        ...
        <tbody>
            @forelse ($products as $product)
                <tr>
                        ...
                        <div class="d-flex align-items-center gap-3">
                            <div class="symbol symbol-35px symbol-circle flex-shrink-0">
                                @if($product->imgThumbPath)
                                    <img src="{{ $product->imgThumbPath }}" class="rounded-circle object-fit-cover w-35px h-35px border" alt="{{ $product->name }}" />
                                @else
                                    <div class="symbol-label bg-soft-primary text-primary fw-bold fs-7 rounded-circle w-35px h-35px d-flex align-items-center justify-content-center">
                    ...
                    <!-- Status (Front Dashboard Signature Legend Indicator) -->
                    <td>
                        ...
                        <div class="d-inline-flex align-items-center justify-content-end gap-2">
                            ...
                            @can('basicdata.products.destroy')
                                {!! Form::open(['route' => ['basicdata.products.destroy', $product->id], 'method' => 'delete', 'class' => 'd-inline']) !!}
                                    {!! Form::button('<i class="fa-solid fa-trash text-danger fs-8"></i>', [
                                        'type' => 'submit',
                ...
                <tr>
                    <td colspan="8" class="text-center py-10">
                        <div class="d-flex flex-column align-items-center justify-content-center text-muted">
                            ...
                            <span class="fs-7 fw-semibold">لا توجد سجلات متاحة</span>
                        </div>
                    </td>
                </tr>
            @endforelse
