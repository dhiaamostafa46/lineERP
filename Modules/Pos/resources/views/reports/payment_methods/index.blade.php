@extends('layouts.app')
@section('title', __('pos::reports.types.payment_methods'))
@section('styles')
<style>
    @media print {
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">@lang('pos::reports.types.payment_methods')</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-500 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-muted">@lang('pos::reports.types.payment_methods')</li>
                </ul>
            </div>
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            <div class="card shadow-sm my-3" id="card-filter">
                ...
                <div id="kt_docs_card_collapsible" class="collapse show">
                    {!! Form::open(['route' => 'pos.reports.product_sales', 'method' => 'GET']) !!}
                    <div class="card-body">
                        <div class="row">
                    ...
                    <div class="card-footer py-4">
                        ...
                        @endif
                        <a href="{{ route('pos.reports.payment_methods') }}" class="btn btn-sm btn-btc float-right"><i class="fa-solid fa-circle-xmark me-1"></i></a>
                    </div>
                    {!! Form::close() !!}
                </div>
            </div>
            @if($reports !== null)
                    ...
                    <div class="col-md-12">
                        <div class="card bg-light-success shadow-sm">
                            <div class="card-body p-4 text-center">
                                <h4 class="text-success mb-2">@lang('pos::reports.total_collections_methods')</h4>
                                <h2 class="fw-bolder">{{ number_format($reports->grand_totals->grand_amount ?? 0, 2) }}</h2>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card shadow-sm">
                    <div class="card-header"><h3 class="card-title">@lang('pos::reports.types.payment_methods') <span class="badge badge-light-primary ms-2">{{ $reports->total() }} @lang('lang.record')</span></h3></div>
                    <div class="card-body pt-3">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover align-middle">
                                <thead class="bg-light-primary"><tr class="fw-bold text-gray-800">
                                    <th class="text-center">#</th>
                                    <th>@lang('pos::lang.payment_method')</th>
                                    <th class="text-end">@lang('pos::reports.total_amount_col')</th>
                                </tr></thead>
                                <tbody>
                                    @forelse($reports as $report)
                                        <tr>
                                            <td class="text-center">{{ ($reports->currentPage()-1) * $reports->perPage() + $loop->iteration }}</td>
                                            <td>{{ $report->payment_method_name ?? $report->payment_method_code }}</td>
                                            
                                            <td class="text-end fw-bold text-primary">{{ number_format($report->total_amount, 2) }}</td>
                                        </tr>
                                    @empty
                                        <tr><td colspan="3" class="text-center py-10">@lang('lang.no_data')</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
