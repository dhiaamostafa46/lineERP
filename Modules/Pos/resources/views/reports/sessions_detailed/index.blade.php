@extends('layouts.app')
@section('title', __('pos::reports.types.sessions_detailed'))
@section('styles')
<style>
    @media print {
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">@lang('pos::reports.types.sessions_detailed')</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-500 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-muted">@lang('pos::reports.types.sessions_detailed')</li>
                </ul>
            </div>
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            <div class="card shadow-sm my-3" id="card-filter">
                ...
                <div id="kt_docs_card_collapsible" class="collapse show">
                    ...
                    <div class="card-footer py-4">
                        ...
                        @endif
                        <a href="{{ route('pos.reports.sessions_detailed') }}" class="btn btn-sm btn-btc float-right"><i class="fa-solid fa-circle-xmark me-1"></i></a>
                    </div>
                    {!! Form::close() !!}
                </div>
            </div>
            @if($reports !== null)
                    ...
                    <div class="col-md-12">
                        <div class="card bg-light-success shadow-sm">
                            <div class="card-body p-4 text-center">
                                <h4 class="text-success mb-2">@lang('pos::reports.total_session_sales_vat')</h4>
                ...
                <div class="card shadow-sm">
                    <div class="card-header"><h3 class="card-title">@lang('pos::reports.types.sessions_detailed') <span class="badge badge-light-primary ms-2">{{ $reports->total() }} @lang('lang.record')</span></h3></div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover align-middle">
                                <thead class="bg-light-primary"><tr class="fw-bold text-gray-800">
                                    ...
                                    <th>@lang('pos::lang.session')</th>
                                    <th class="text-end">@lang('pos::lang.total')</th>
                                </tr></thead>
                                <tbody>
                                    @forelse($reports as $inv)
                                        <tr>
                                            <td class="text-center">{{ ($reports->currentPage()-1) * $reports->perPage() + $loop->iteration }}</td>
                                            <td>{{ $inv->invoice_number }}</td>
                                            <td>{{ $inv->issue_date->format('Y-m-d') }}</td>
                                            <td>{{ $inv->user?->name }}</td>
                                            <td>{{ $inv->pos_session_id ?? '---' }}</td>
